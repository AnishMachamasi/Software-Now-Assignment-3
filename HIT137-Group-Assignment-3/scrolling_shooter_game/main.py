...
jump_fx = pygame.mixer.Sound("./sounds/audio/jump.wav")
jump_fx.set_volume(0.05)
shot_fx = pygame.mixer.Sound("./sounds/audio/shot.wav")
shot_fx.set_volume(0.05)
grenade_fx = pygame.mixer.Sound("./sounds/audio/grenade.wav")
        ...
        if player.alive:
            ...
            if level_complete:
                ...
                if level <= MAX_LEVELS:
                    # load in level data and create world
                    with open(
                        f"./level_data/level{level}_data.csv", newline=""
                    ) as csvfile:
                        reader = csv.reader(csvfile, delimiter=",")
                        for x, row in enumerate(reader):
                            for y, tile in enumerate(row):
